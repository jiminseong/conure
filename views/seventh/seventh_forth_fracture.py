...
def display_page7_4():
    ...
    st.markdown(f'<img src="data:image/jpeg;base64,{img_base64}" alt="척추 이미지" style="width:100%; max-width:600px;">', unsafe_allow_html=True)

    # 증상 선택 옵션 제공
    st.markdown("척추 골절이 예상되는 부위를 선택해주세요.")
    symptom_options = [
        "경추(목뼈)",
        "흉추(등뼈)",
        "요추(허리뼈)",
    ...
    if submit:
        ...
        #     model="gpt-4",
        # )
        st.write(f"아픈 부위: 척추-{selected_symptom}")
